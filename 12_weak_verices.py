...
class SimpleGraph:
    ...
    def BreadthFirstSearch(self, VFrom, VTo):
        ...
        while queue:
            path = queue.pop(0)
            el_number = path[-1]
            self.vertex[el_number].Hit = True
            if el_number == VTo:
                path_vertex = []
                for i in path:
                    path_vertex.append(self.vertex[i])
                return path_vertex
            else:
                for i in range(self.max_vertex):
                    if self.m_adjacency[el_number][i] == 1 and self.vertex[i].Hit == False:
                        new_path = list(path)
                        new_path.append(i)
        ...
        def isWeak(vert):
            ...
            for i in range(self.max_vertex):
                if self.m_adjacency[vert][i] == 1:
                    neighbours.append(i)
            while neighbours:
                neib = neighbours.pop(0)
                for index in neighbours:
                    if self.m_adjacency[neib][index] == 1:
                        for name in (vert, neib, index):
                            self.vertex[name].Hit = True

        for i in range(self.max_vertex):
            if self.vertex[i] is not None and self.vertex[i].Hit == False:
                isWeak(i)
        result = []
        for i in range(self.max_vertex):
